...
export default function Schedule() {
  ...
  const { data: allGames, isLoading, error } = useQuery<Game[]>({
    queryKey: ["/api/games/all", seasonNumber],
    queryFn: async () => {
      const res = await fetch(`/api/games/all?season=${seasonNumber}`);
  ...
  const getWeekLabel = (w: number) => {
    const names = activeSeason?.weekNames as Record<string, string> | undefined;
    return names?.[String(w)] || `Week ${w}`;
  };
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-4">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black italic uppercase tracking-tighter leading-[0.9]">
            Schedule <span className="text-muted-foreground/20">S{seasonNumber}</span>
          </h1>
        </div>
