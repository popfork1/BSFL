    ...
    console.log('Initializing database schema...');
      ...
      await rawSql`
        CREATE TABLE IF NOT EXISTS seasons (
          ...
          week_count INTEGER NOT NULL DEFAULT 15,
          week_names JSONB DEFAULT '{}',
      ...
      try {
        await rawSql`ALTER TABLE games ADD COLUMN season INTEGER DEFAULT 1`;
        console.log('Added season column to games table');
      } catch (err) {
        // Column likely already exists
      }

      // Add season column to standings table if it doesn't exist
      try {
        await rawSql`ALTER TABLE standings ADD COLUMN season INTEGER DEFAULT 1`;
        console.log('Added season column to standings table');
      } catch (err) {
        // Column likely already exists
      }
