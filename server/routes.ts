...
import {
  ...
  insertPowerRankingSchema,
  insertHallOfFameSchema,
  insertSeasonSchema,
} from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/games", async (req, res) => {
    try {
      ...
      const season = seasonStr ? parseInt(seasonStr) : 1;
  ...
  app.patch("/api/standings/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const updated = await storage.updateStandings(req.params.id, req.body);
      res.json(updated);
    } catch (error) {
      console.error("Error updating standing:", error);
      res.status(500).json({ message: "Failed to update standing" });
    }
  });
  ...
  app.patch("/api/player-stats/:id", isAuthenticated, async (req: any, res) => {
    try {
      if (req.session?.role !== "admin") {
        return res.status(403).json({ message: "Admin access required" });
      }
      const updated = await (storage as any).updatePlayerStats(req.params.id, req.body);
      res.json(updated);
    } catch (error) {
  ...
  app.get("/api/players", async (req, res, next) => {
    try {
      ...
      console.error("[API] Error fetching all players:", error);
      next(error);
    }
  });
  ...
  app.get("/api/seasons", async (req, res) => {
    try {
      const all = await storage.getAllSeasons();
      res.json(all);
    } catch (error) {
  ...
  app.post("/api/seasons/:id/activate", isAuthenticated, async (req, res) => {
    try {
      await storage.setActiveSeason(req.params.id);
      res.json({ success: true });
    } catch (error) {
  ...
  app.delete("/api/seasons/:id", isAuthenticated, async (req, res) => {
    try {
  ...
  app.get("/api/power-rankings", async (req, res) => {
    try {
      ...
      const rankings = await storage.getPowerRankings(week);
      res.json(rankings);
    } catch (error) {
  ...
  app.get("/api/power-rankings/current-week", async (req, res) => {
    try {
      const week = await storage.getCurrentPowerRankingsWeek();
      res.json({ week });
    } catch (error) {
  ...
  app.post("/api/power-rankings", isAuthenticated, async (req, res) => {
    try {
      const data = insertPowerRankingSchema.parse(req.body);
      const ranking = await storage.upsertPowerRanking(data);
      res.json(ranking);
    } catch (error) {
      res.status(400).json({ message: "Invalid ranking data" });
    }
  });

  app.delete("/api/power-rankings/:id", isAuthenticated, async (req, res) => {
    try {
      await storage.deletePowerRanking(req.params.id);
      res.json({ success: true });
    } catch (error) {
  ...
  app.post("/api/hall-of-fame", isAuthenticated, async (req, res) => {
    try {
      const data = insertHallOfFameSchema.parse(req.body);
      const entry = await storage.createHallOfFameEntry(data);
      res.json(entry);
    } catch (error) {
  ...
  app.patch("/api/hall-of-fame/:id", isAuthenticated, async (req, res) => {
    try {
  ...
  app.delete("/api/hall-of-fame/:id", isAuthenticated, async (req, res) => {
    try {
