...
export interface IStorage {
  ...
  updateSeason(id: string, season: Partial<Season>): Promise<Season>;
  deleteSeason(id: string): Promise<void>;
  setActiveSeason(id: string): Promise<void>;

  // Power Rankings
  getPowerRankings(week?: number): Promise<PowerRanking[]>;
  upsertPowerRanking(ranking: InsertPowerRanking): Promise<PowerRanking>;
  deletePowerRanking(id: string): Promise<void>;
  getCurrentPowerRankingsWeek(): Promise<number>;

  // Hall of Fame
  getAllHallOfFame(): Promise<HallOfFameEntry[]>;
  createHallOfFameEntry(entry: InsertHallOfFame): Promise<HallOfFameEntry>;
  updateHallOfFameEntry(id: string, entry: Partial<HallOfFameEntry>): Promise<HallOfFameEntry>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getCurrentWeekGames(): Promise<Game[]> {
    ...
    const latestSeason = activeSeason?.number ?? 1;
  ...
  async updateStandings(id: string, data: Partial<Standings>): Promise<Standings> {
    const clean = cleanObject(data);
    const [updated] = await db.update(standings).set(clean).where(eq(standings.id, id)).returning();
    return updated;
  }
  ...
  async updatePlayerStats(id: string, statsData: Partial<PlayerStats>): Promise<PlayerStats> {
    const cleanData = cleanObject(statsData);
    const [updated] = await db.update(playerStats).set(cleanData).where(eq(playerStats.id, id)).returning();
    return updated;
  }
  ...
  async updateTeam(id: string, data: Partial<Team>): Promise<Team> {
    const [team] = await db.update(teams).set(cleanObject(data)).where(eq(teams.id, id)).returning();
    return team;
  }
  ...
  async createSeason(season: InsertSeason): Promise<Season> {
    const result = await db.insert(seasons).values(cleanObject(season)).returning();
    return result[0];
  }

  async updateSeason(id: string, season: Partial<Season>): Promise<Season> {
    const result = await db.update(seasons).set(cleanObject(season)).where(eq(seasons.id, id)).returning();
    return result[0];
  }
  ...
  async getPowerRankings(week?: number): Promise<PowerRanking[]> {
    if (week !== undefined) {
      return db.select().from(powerRankings).where(eq(powerRankings.week, week));
    }
    return db.select().from(powerRankings);
  }

  async upsertPowerRanking(ranking: InsertPowerRanking): Promise<PowerRanking> {
    const result = await db.insert(powerRankings).values(cleanObject(ranking)).returning();
    return result[0];
  }

  async deletePowerRanking(id: string): Promise<void> {
    await db.delete(powerRankings).where(eq(powerRankings.id, id));
  }

  async getCurrentPowerRankingsWeek(): Promise<number> {
    const result = await db
      .select({ week: powerRankings.week })
      .from(powerRankings)
      .orderBy(desc(powerRankings.week))
      .limit(1);
    return result[0]?.week ?? 1;
  }

  async getAllHallOfFame(): Promise<HallOfFameEntry[]> {
    return db.select().from(hallOfFame).orderBy(hallOfFame.displayOrder, hallOfFame.createdAt);
  }

  async createHallOfFameEntry(entry: InsertHallOfFame): Promise<HallOfFameEntry> {
    const result = await db.insert(hallOfFame).values(cleanObject(entry)).returning();
    return result[0];
  }

  async updateHallOfFameEntry(id: string, entry: Partial<HallOfFameEntry>): Promise<HallOfFameEntry> {
    const result = await db.update(hallOfFame).set(cleanObject(entry)).where(eq(hallOfFame.id, id)).returning();
    return result[0];
  }
