...
export const insertNewsSchema = createInsertSchema(news).omit({
  ...
  publishedAt: z.union([z.string().transform(s => new Date(s)), z.date(), z.null(), z.undefined()]).optional(),
});
...
export const seasons = pgTable("seasons", {
  ...
  name: varchar("name", { length: 100 }).notNull(),
  weekCount: integer("week_count").notNull().default(15),
  weekNames: jsonb("week_names").default({}),
...
export const powerRankings = pgTable("power_rankings", {
  ...
  trend: varchar("trend", { length: 10 }).default("neutral"),
  trendAmount: integer("trend_amount").default(0),
  note: text("note"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export const insertPowerRankingSchema = createInsertSchema(powerRankings).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertPowerRanking = z.infer<typeof insertPowerRankingSchema>;
export type PowerRanking = typeof powerRankings.$inferSelect;
...
export const hallOfFame = pgTable("hall_of_fame", {
  ...
  inductedYear: integer("inducted_year"),
